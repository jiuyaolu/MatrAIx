...
persona compares all four **Network & CDN** plans and recommends the one that
best fits the organization context represented in their persona.
...
uv run harbor run \
  ...
  -p application/tasks/web-playwright-cloudflare-plan-comparison
```
...
and the submitted comparison. Persona alignment is reported separately from
