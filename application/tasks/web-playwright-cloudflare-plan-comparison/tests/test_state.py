...
def _load() -> dict[str, object]:
    ...
    assert isinstance(data, dict), (
        "cloudflare_plan_comparison.json root must be an object"
    )
    return data
...
def test_output_schema() -> None:
    ...
    assert data.get("task_pricing_category") == PRICING_CATEGORY, (
        "task_pricing_category must be Network & CDN"
    )
    ...
    assert set(ids) == set(PLAN_LABELS), (
        "candidate list must contain Free, Pro, Business, and Contract"
    )
    ...
    if feedback is not None:
        ...
        for key, label in (
            ("trust_level", "Recommendation confidence"),
            ...
            feedback_facets.append(
                {
                    ...
                    "label": "Clarity of recommendation",
                    "role": "evidence",
                    "kind": "categorical",
